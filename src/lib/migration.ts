...
export async function getStorageMetadata(): Promise<StorageMetadata> {
...
export async function setStorageMetadata(metadata: StorageMetadata): Promise<void> {
  await set(STORAGE_KEYS.METADATA, metadata);
}

export function needsMigration(metadata: StorageMetadata): boolean {
...
export function migrateLegacyPhrase(legacy: LegacyPhrase, migrationMap: MigrationMap): PhraseEntity {
...
export function migrateIdReferences(ids: string[], migrationMap: MigrationMap): string[] {
  return ids.map(id => migrationMap[id] || id);
}

export function migrateLearningStatus(
  status: LearningStatus,
  migrationMap: MigrationMap
): LearningStatus {
  return {
    ...status,
    completedIds: migrateIdReferences(status.completedIds, migrationMap),
    incorrectIds: migrateIdReferences(status.incorrectIds, migrationMap),
  ...
  try {
    ...
    for (const phrase of phraseList) {
      if (isV2Phrase(phrase)) {
        ...
        const migrated = migrateLegacyPhrase(phrase, migrationMap);
    ...
    if (learningStatus && Object.keys(migrationMap).length > 0) {
      const migratedStatus = migrateLearningStatus(learningStatus, migrationMap);
    ...
    await setStorageMetadata(newMetadata);
...
export async function getMigrationMap(): Promise<MigrationMap> {
